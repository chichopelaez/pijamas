      ...
      <table className="min-w-full divide-y divide-gray-700">
        ...
        <tbody className="bg-gray-800 divide-y divide-gray-700">
          {products.map((product, index) => (
            <tr key={index} className={`hover:bg-gray-700 transition-colors duration-200 ${!product.activo ? 'opacity-50' : ''}`}>
              <td className="px-6 py-4 whitespace-nowrap">
                <div className="font-medium text-gray-100">{product.nombre_producto}</div>
                <div className="text-sm text-gray-400">{product.descripcion?.substring(0, 30)}...</div>
              </td>
              ...
              <td className="px-6 py-4 whitespace-nowrap">
                <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full 
                  ${product.stock_actual <= product.stock_minimo ? 'bg-red-800 text-red-200' : 'bg-green-800 text-green-200'}`}>
